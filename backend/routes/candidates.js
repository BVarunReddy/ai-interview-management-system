  ...
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO candidates (name, email, phone, position, experience, skills, resume_path, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, email, phone || null, position, parseInt(experience) || 0, skills || null, resumePath, req.user.id]
    );
    await notify(
      "New Candidate Added",
    ...
    res.status(201).json({ success: true, message: "Candidate added successfully.", candidateId: result.insertId });
  } catch (err) {
...
router.get("/", async (req, res) => {
  try {
    ...
    const [[{ total }]] = await db.query(`SELECT COUNT(*) AS total FROM candidates c ${where}`, params);
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const [interviews] = await db.query("SELECT * FROM interviews WHERE candidate_id = ? ORDER BY interview_date DESC", [req.params.id]);
    const [feedback] = await db.query("SELECT * FROM feedback WHERE candidate_id = ? ORDER BY created_at DESC", [req.params.id]);
...
router.put("/:id/status", async (req, res) => {
  try {
    ...
    const valid = ["Pending","Screening","Interview","Offer","Selected","Rejected"];
    if (!valid.includes(status)) return res.status(400).json({ success: false, message: "Invalid status." });
    const [rows] = await db.query("SELECT name, position FROM candidates WHERE id = ?", [req.params.id]);
    if (!rows.length) return res.status(404).json({ success: false, message: "Candidate not found." });
    await db.query("UPDATE candidates SET status = ? WHERE id = ?", [status, req.params.id]);
    const typeMap = { Selected:"success", Rejected:"error", Offer:"success" };
    await notify(
...
router.put("/:id/ai-score", async (req, res) => {
  try {
    ...
    await db.query("UPDATE candidates SET ai_score = ? WHERE id = ?", [ai_score, req.params.id]);
...
router.delete("/:id", async (req, res) => {
  try {
    const [rows] = await db.query("SELECT resume_path, name FROM candidates WHERE id = ?", [req.params.id]);
